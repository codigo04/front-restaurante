...
export const savePedido = async (pedido) => {
  try {
    const response = await axios.post(
      `${BASE_URL}/api/v1/admin/pedidos/crear`,
      pedido,
      configToken
    );
...
export const saveDetallePedido = async (detallePedido) => {
  try {
    const response = await axios.post(
      `${BASE_URL}/api/v1/admin/detalle-pedidos/saveAll`,
      detallePedido,
      configToken
    );
    return response.data;
...
export const obtenerPedido = async (pedidoId) => {
    ...
    detallePedidos: [
      {
        ...
        producto: {
          ...
          imagen:
            "https://images.unsplash.com/photo-1667204651371-5d4a65b8b5a9?fm=jpg&q=60&w=3000&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTZ8fGNvY2ElMjBjb2xhfGVufDB8fDB8fHww",
          estado: "ACTIVO",
          porcion: "",
          stock: 100,
          litros: "500",
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/v1/admin/pedidos/${pedidoId}`,
      configToken
    );
...
export const getPedidos = async () => {
  try {
    const response = await axios.get(
      `${BASE_URL}/api/v1/admin/pedidos/todos`,
      configToken
    );
    return response.data;
  } catch (error) {
...
export const actualizarEstadoPedido = async (pedidoId, estado) => {
  ...
  try {
    const response = await axios.put(
      `${BASE_URL}/api/v1/admin/pedidos/actualizar/estado/${pedidoId}`,
      newEstado,
      configToken
    );
...
//Monto de pedidos por dia
export const obtenerTotalVentasHoy = async () => {
  try {
    const response = await axios.get(
      `${BASE_URL}/api/v1/admin/pedidos/total-pagado-hoy`,
      configToken
    );
    if (!response.data) {
